  ...
  const data = useMemo(() => {
    if (enrichedBondingChanges && tokenBonding && baseMint && targetMint && curve) {
      ...
      for (let c of changes) {
        ...
        if (isNaN(price) || price === Infinity) continue;
      ...
      return result.filter(p => !isNaN(p.price) && p.price !== Infinity);
    }
    return [];
  }, [enrichedBondingChanges, tokenBonding, baseMint, targetMint, curve, stopTime, numDataPoints]);
